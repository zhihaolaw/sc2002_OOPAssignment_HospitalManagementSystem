...
public class Patient extends User implements CSV_PathHandler,LoginSystem {
    ...
    public void updateContactInfo() {
        ...
        viewMedicalRecord(this.hospitalID);
    }

    // Method to update only the phone and email for the specific patient in the CSV
    private void updatePatientContactInCSV() {
    	File file = new File(PATIENT_LIST_FILE_PATH); 
        
        // Step 1: Check if the file exists 
        if (!file.exists()) { 
            System.out.println("Patient data file not found at " + PATIENT_LIST_FILE_PATH); 
            return; 
        } 
        ...
        // Step 3: Update the specific patient's contact information in memory 
        for (String[] record : records) { 
            if (record[0].equals(this.hospitalID)) {  // Check if hospitalID matches 
                record[7] = this.contactNumber;       // Update phone number (assuming index 7) 
                record[9] = this.email;               // Update email address (assuming index 9) 
                patientFound = true; 
                break; 
            } 
        } 
 
        // Step 4: If patient is found, save the updated records back to the file 
        if (patientFound) { 
            String header = "Patient ID,Name,Date of Birth,Gender,Role,Age,Blood Type,Phone Number,Password,Email Address,Past Diagnosis,Past Treatments"; 
...
//        
    ...
    public static String getSymptomsFromUser() {
    ...
    public void showMenu() {
        ...
        do {
            ...
            System.out.println("10. Symptom Checker");
            System.out.println("11. Schedule Emergency Appointment");
            ...
            switch (choice) {
                ...
                case 9:
                    ...
                    break;
                case 10:
                	String symptoms = getSymptomsFromUser();
                    String response = SymptomChecker.sendSymptomDataToApi(symptoms);
                    SymptomChecker.processResponse(response);
                    break;
                case 11: 
                    testAppointment.scheduleEmergencyAppointment(this.hospitalID,this.name);
                    break;
                case 12:
