...
public List<String[]> loadScheduledAppointments(String DoctorID) {
        ...
        //boolean hasAvailableSlots = false;
      	...
      	try(BufferedReader br = new BufferedReader(new FileReader(APPOINTMENTS_FILE_PATH)))
      	{
      		...
      		while((line = br.readLine()) != null)
      		{
                  ...
                  if(patientId.equals(PatientID) && (status.equals("Scheduled") || status.equals("Pending")))
                  {
        ...
        if (availableSlots.isEmpty()) {
            ...
            for (String[] slot : availableSlots) {
                System.out.printf("Appointment ID: %s | Date: %s | Time: %s | Doctor: %s | Status: %s\n",
                        slot[0], slot[4], slot[5], slot[3], slot[6]);
            }
        }
    }
    ...
    public void scheduleEmergencyAppointment(String Pid,String pName) {
        Scanner sc = new Scanner(System.in);
        System.out.println("You are about to schedule an emergency appointment.");

        // Ask for patient details
        System.out.print("Enter the reason for the emergency: ");
        String emergencyReason = sc.nextLine();

        // Send alert to Telegram
        String alertMessage = "🚨 Emergency Alert: A patient requires immediate attention.\nReason: " + emergencyReason + 
            ", Patient Id: " + Pid + ", Patient Name: " + pName;
        TelegramBotService.sendTelegramMessage(alertMessage);

        // Schedule the emergency appointment (custom logic)
        System.out.println("The medical team has been notified.");
